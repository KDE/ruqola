...
#include "restapi/chat/reactonmessagejob.h"
...
void ReactOnMessageJobTest::shouldGenerateJson()
{
    ...
    job.setEmoji(emoji);
    QCOMPARE(job.json().toJson(QJsonDocument::Compact), QStringLiteral("{\"emoji\":\"%1\",\"messageId\":\"%2\"}").arg(emoji).arg(messageid).toLatin1());
}
