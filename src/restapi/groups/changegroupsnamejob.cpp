...
bool ChangeGroupsNameJob::start()
{
    ...
    connect(reply, &QNetworkReply::finished, this, &ChangeGroupsNameJob::slotChangeTopicFinished);
    return true;
}

void ChangeGroupsNameJob::slotChangeTopicFinished()
{
...
{
    if (mName.isEmpty()) {
        qCWarning(RUQOLA_RESTAPI_LOG) << "ChangeGroupsNameJob: name is empty";
        return false;
    }
    if (mRoomId.isEmpty()) {
        qCWarning(RUQOLA_RESTAPI_LOG) << "ChangeGroupsNameJob: RoomId is empty";
        return false;
    }
    if (!RestApiAbstractJob::canStart()) {
        qCWarning(RUQOLA_RESTAPI_LOG) << "Impossible to start ChangeGroupsNameJob job";
        return false;
    }
    return true;
}
