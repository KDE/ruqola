...
bool ChannelRemoveLeaderJob::start()
{
    ...
    addLoggerInfo("ChannelRemoveLeaderJob::start: " + baPostData);
...
bool ChannelRemoveLeaderJob::canStart() const
{
    ...
    if (mRoomId.isEmpty()) {
        qCWarning(ROCKETCHATQTRESTAPI_LOG) << "ChannelRemoveLeaderJob: RoomId is empty";
        return false;
    }
...
QString ChannelRemoveLeaderJob::roomId() const
{
    return mRoomId;
}

void ChannelRemoveLeaderJob::setRoomId(const QString &roomId)
{
    mRoomId = roomId;
}
