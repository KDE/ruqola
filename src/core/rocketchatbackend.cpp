...
void RocketChatBackend::slotDDPConnectedChanged(bool connected)
{
    if (connected) {
        ...
        auto restApi = mRocketChatAccount->restApi();
        if (restApi && restApi->authenticationManager()->isLoggedIn()) {
            ddpLogin();
        }
    }
}
...
void RocketChatBackend::slotChanged(const QJsonObject &object)
{
    ...
    if (collection == "stream-room-messages"_L1) {
        qCDebug(RUQOLA_UNKNOWN_COLLECTIONTYPE_LOG) << " RocketChatBackend::slotChanged stream-room-messages " << contents;
        ...
        if (eventname.endsWith("/uiInteraction"_L1)) {
            ...
            if (actionName == "updated"_L1) {
                ...
                qDebug() << " gfghgffdghfdghgfhfghgfhgdf" << roomId;
                if (auto r = model->findRoom(roomId); r) {
                    mRocketChatAccount->addRoomToDataBase(r);
                    qDebug() << " cccccsdfsdfsdfsdfsdfsdfsdfdsfdsfsdfsdf";
                    qDebug() << " cccccsdfsdfsdfsdfsdfsdfsdfdsfdsfsdfsdf1111 " << r;
                    mRocketChatAccount->addNewRoomNotification(r);
                }
                qDebug() << " CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCc";
