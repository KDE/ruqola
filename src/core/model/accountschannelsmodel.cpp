    ...
    : QAbstractItemModel(parent)
{
    ...
    mapModelToIndex(acctsProxy, []{ return QModelIndex(); });
    for (int i = 0, count = acctsProxy->rowCount(); i < count; ++i)
        mapRoomsModel(i);
}
...
{
    const auto find = [model](const ProxyIndex &i){ return i.model == model; };
...
{
    const auto find = [&root](const ProxyIndex &i){ return i.root() == root; };
    const auto it = std::find_if(mProxied.begin(), mProxied.end(), find);
